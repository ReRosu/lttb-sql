...
CREATE OR REPLACE FUNCTION timestamp_diff(ft timestamp with time zone, st timestamp with time zone, r_t integer DEFAULT 0)
 RETURNS float8
 LANGUAGE plpgsql
AS $function$
declare 
...
CREATE OR REPLACE FUNCTION lttb(left_b timestamp with time zone, right_b timestamp with time zone, id_of_well integer, column_num integer, n_out integer)
 RETURNS TABLE(x timestamp with time zone, y double precision)
 LANGUAGE plpgsql
AS $function$
declare
	not_sampled ts_dot[];
	...
	area_ float8; 
begin
	not_sampled := array(select (dt, "values"[column_num])::ts_dot from public.wells_data where 
		(dt between left_b and right_b) and 
		well_id = id_of_well and not ("values"[column_num] = double precision 'NaN')
		order by dt asc);
	if array_length(not_sampled, 1) <= n_out then
		return query select * from unnest(not_sampled);
		return;
	end if;
	start_tz := (not_sampled[1].x)::timestamptz;
...
--		avg_x := avg_range_length;
		...
		begin loop
			avg_x := avg_x + public.timestamp_diff(not_sampled[j].x::timestamptz, start_tz, 1);
        ...
        -- Point a
        point_ax := public.timestamp_diff(not_sampled[a].x::timestamptz, start_tz, 1);
